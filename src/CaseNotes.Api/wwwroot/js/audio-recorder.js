...
window.stopRecording = async function(dotNetHelper) {
        ...
        mediaRecorder.onstop = async () => {
            try {
                ...
                reader.onloadend = async () => {
                    ...
                    console.log(`Recording stopped. Audio base64 length: ${base64.length}`);
                    
                    // HIPAA: Audio is only in memory, never written to disk
                    // Clear the chunks array
                    audioChunks = [];
                    
                    // Invoke C# method to receive the base64 string
                    await dotNetHelper.invokeMethodAsync('ReceiveAudioData', base64);
                    resolve();
                };
                reader.onerror = () => {
